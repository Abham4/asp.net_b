...
{
    public class CreateStaffAddress : IRequest<string>
    {
    ...
    public class CreateStaffAddressHandler : IRequestHandler<CreateStaffAddress, string>
    {
        private readonly IStaffRepository _staffRepo;
        private readonly IAddressRepository _addressRepo;
        private readonly IUserRepository _userRepo;
        private readonly ILogger<CreateStaffAddressHandler> _logger;

        public CreateStaffAddressHandler(IStaffRepository staffRepository, IAddressRepository addressRepository,
            ILogger<CreateStaffAddressHandler> logger, IUserRepository userRepository)
        {
        ...
        public async Task<string> Handle(CreateStaffAddress request, CancellationToken cancellationToken)
        {
            var user = await _userRepo.GetAuthenticatedUser();
            var staff = await _staffRepo.GetByIdAsync(request.Reference);

            if (staff == null)
                throw new DomainException("Refered staff doesn't exist!");
            ...
            var address = new Domain.Entities.Address("Staff", addressType, request.Value, attribute, request.Reference,
                user.Email);

            _logger.LogInformation("---------Adding new staff address----------");
    ...
    public class CreateStaffAddressValidator : AbstractValidator<CreateStaffAddress>
    {
        [Obsolete]
        public CreateStaffAddressValidator()
        {
